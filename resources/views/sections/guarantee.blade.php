        ...
        <div class="lp-card-grid">
            @foreach (data_get($section, 'content.items', []) as $item)
                <article class="lp-card">
                    <div class="lp-card-number">G</div>
                    ...
                    <p class="lp-card-copy" @if($isEditor) contenteditable="true" @input="{{ $ref }}.content.items[{{ $loop->index }}].copy = $event.target.innerText" x-text="{{ $ref }}.content.items[{{ $loop->index }}].copy" @endif>{{ data_get($item, 'copy') }}</p>
                </article>
            @endforeach
        </div>
    </div>
